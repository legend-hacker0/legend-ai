...
async function handleImageGenerationRequest(
  ...
  try {
    ...
    const runSchnell = async (): Promise<any> =>
    ...
    // Only use another model for technical/transient failures. Do not keep sending
    // a prompt that Workers AI has explicitly rejected by its safety filter.
    ...
    if (safetyRejected) {
      console.warn("Legend AI image prompt rejected by Workers AI safety filtering.", errors);
      return jsonResponse({
        error: "This image prompt was rejected by the image model's safety filter. Please change the prompt and try again.",
