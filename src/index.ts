...
const IMAGE_EDIT_MODEL_ID = "black-forest-labs/flux-1-kontext-pro";
...
async function handleImageEditRequest(
  ...
  try {
    ...
    const editPrompt =
      `You are editing an uploaded image for Legend AI. The user's requested change is: ${prompt}. ` +
      `Preserve the original image's composition, subject identity, pose, framing, background, lighting, texture, and all other details unless the request specifically asks to change them. ` +
      `Make the requested modification clearly and naturally. Current user: ${playerName}.`;
