...
async function handleChatRequest(request: Request, env: Env): Promise<Response> {
  try {
    ...
    return new Response(JSON.stringify({ error: "Legend AI could not process your request." }), {
      status: 500,
      headers: { ...CORS_HEADERS, "Content-Type": "application/json" },
    });
  }
...
async function handleFileRequest(request: Request, env: Env): Promise<Response> {
  try {
    ...
    return new Response(
      JSON.stringify({ success: true, files: results, count: results.length }),
      {
        status: 200,
        headers: { ...CORS_HEADERS, "Content-Type": "application/json" },
      },
    ...
    return new Response(JSON.stringify({ error: "Legend AI could not read this file." }), {
      status: 500,
      headers: { ...CORS_HEADERS, "Content-Type": "application/json" },
    });
  }
