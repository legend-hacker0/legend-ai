...
async function handleImageGenerationRequest(
  ...
  try {
    ...
    const runKlein = async (modelId: string): Promise<any> => {
      ...
      if (!formStream || !formContentType) throw new Error("Could not prepare the multipart image request.");
    ...
    const runSchnell = async (): Promise<any> => env.AI.run("@cf/black-forest-labs/flux-1-schnell", {
      prompt,
      steps: 4,
    });
    ...
    // Primary model: FLUX.2 Klein 4B, using Cloudflare's documented multipart format.
    try {
      result = await runKlein(IMAGE_GENERATION_MODEL_ID);
    } catch (error) {
      errors.push(`Klein 4B: ${error instanceof Error ? error.message : String(error)}`);
    }

    // If Klein 4B has a transient/model-specific failure, try Klein 9B without changing
    // the user's prompt. Both Klein models use the same documented multipart interface.
    if (!result) {
      try {
        result = await runKlein(IMAGE_GENERATION_FALLBACK_MODEL_IDS[0]);
      } catch (error) {
        errors.push(`Klein 9B: ${error instanceof Error ? error.message : String(error)}`);
      }
    }

    // Final compatibility fallback: FLUX.1 schnell uses the standard JSON binding.
    if (!result) {
      try {
        result = await runSchnell();
      } catch (error) {
        errors.push(`FLUX.1 schnell: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
    ...
    if (image instanceof Uint8Array) {
      ...
      for (let i = 0; i < image.length; i += chunkSize) {
        binary += String.fromCharCode(...image.subarray(i, Math.min(i + chunkSize, image.length)));
      }
      return jsonResponse({
        success: true,
    ...
    return jsonResponse({
      error: errors.length
        ? "Image generation failed after trying the available image models. " + errors.join(" | ")
        : "The image models did not return an image.",
    }, 500);
  } catch (error) {
    console.error("Legend AI image generation error:", error);
    return jsonResponse(
      { error: "Legend AI could not generate the image: " + (error instanceof Error ? error.message : String(error)) },
      500,
    );
  }
}
