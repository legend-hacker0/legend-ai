...
async function handleImageGenerationRequest(
  ...
  try {
    ...
    // FLUX.2 Klein 4B officially accepts multipart input on Workers AI.
    // Build a fresh multipart body for every attempt so the request stream is never reused.
    try {
      ...
      if (!formStream || !formContentType) {
        throw new Error("Could not prepare the FLUX.2 multipart request.");
      }

      result = await env.AI.run(IMAGE_GENERATION_MODEL_ID, {
        multipart: {
          body: formStream,
    ...
    // Compatibility fallback for Workers AI runtimes that still accept the
    // older direct parameter form for this model.
    if (!result) {
      try {
        result = await env.AI.run(IMAGE_GENERATION_MODEL_ID, {
          prompt,
        ...
        const first = firstError instanceof Error ? firstError.message : String(firstError || "unknown error");
        const second = error instanceof Error ? error.message : String(error || "unknown error");
        console.error("Legend AI FLUX.2 generation attempts failed:", first, second);
        return jsonResponse({ error: "FLUX.2 Klein 4B failed: " + second }, 500);
      }
    }
    ...
    console.error("Legend AI FLUX.2 returned an unexpected result:", result);
    return jsonResponse({ error: "FLUX.2 Klein 4B did not return an image." }, 500);
  } catch (error) {
